...

{
  ...
  const row = rows.find((r) => /Fremantle/i.test(r.home) && /Brisbane/i.test(r.away));
...

{
  ...
  const row = rows.find((r) => /Azerbaijan Race/i.test(`${r.home} ${r.title}`));
...

{
  ...
  const row = rows.find((r) => /Dolphins/i.test(r.home) && /Roosters/i.test(r.away));
...

{
  ...
  const row = rows.find((r) => /Panthers/i.test(r.home) && /Knights/i.test(r.away));
